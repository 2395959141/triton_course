...
def min_kernel(
    ...
    # 加载m维度上的分块，也就是[pid_m×BLOCK_M, (pid_m+1)×BLOCK_M)
    ...
    for start_n in range(0, N, BLOCK_N):
        # 确定N维上的偏移量，也就是[start_n, start_n+BLOCK_N)
        n_offset = start_n + tl.arange(0, BLOCK_N)
        # 确定当前的偏移量，也就是[m_offset,n_offset]
        offset = m_offset[:, None] * N * K + n_offset[None, :] * K + pid_k
        # 确定当前的掩码，也就是[m_offset,n_offset] < [M,N]
        mask = m_offset[:, None] < M and n_offset[None, :] < N
        inp_ptrs = inp + offset

        # 加载当前的元素，也就是[m_offset,n_offset]
        inp_vals = tl.load(inp_ptrs, mask=mask, other=float("inf"))
        local_min, local_argmin = tl.min(inp_vals, 1, return_indices=True)

        # 和全局的最小值进行比较，如果当前的最小值小于全局的最小值，则更新全局的最小值
