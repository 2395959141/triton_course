...
def skip_rmsnorm_no_view(X, residual, weight, eps=1e-5):
    ...
    skip_rms_norm_kernel_no_view[grid](
        ...
        ),  # 若无residual，这里传X只是占位，kernel中不使用R_ptr
        weight,
        B,
        S,
        N,
...
def skip_rmsnorm(X, residual, weight, eps=1e-5):
    ...
    if residual is not None:
        ...
        skip_rms_norm_kernel[M,](
            Y, X, residual, weight, N, 1, N, 1, N, 1, N, eps, BLOCK_SIZE=BLOCK_SIZE
        )
        return Y.view(orig_shape), residual.view(orig_shape)
    else:
        rms_norm_kernel[M,](Y, X, weight, N, 1, N, 1, N, eps, BLOCK_SIZE=BLOCK_SIZE)
...
if __name__ == "__main__":
    ...
    original_times = benchmark(python_skip_rmsnorm, shapes)
