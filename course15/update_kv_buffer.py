...
def test1():
    ...
    dest1 = torch.zeros((B * Seq_Len, H, D), dtype=torch.float16).cuda()
    dest2 = torch.zeros((B * Seq_Len, H, D), dtype=torch.float16).cuda()

    src = torch.randn((B * Seq_Len, H, D), dtype=torch.float16).cuda()
    dest_loc = torch.arange(0, B * Seq_Len, dtype=torch.int32, device="cuda")
    shuffled_indices = torch.randperm(dest_loc.size(0), device="cuda")
    dest_loc = dest_loc[shuffled_indices]

    for _ in range(10):  # Warm up
        update_kv_buffer(src, dest_loc, dest1)
    ...
    print("max ", torch.max(torch.abs(dest1 - dest2)))
    print("mean ", torch.mean(torch.abs(dest1 - dest2)))
    assert torch.allclose(dest1, dest2, atol=1e-2, rtol=0)
