...
def _flash_decoding_stage1_kernel(
    ...
    # 1000 个块，每个块有5个小块，分成200个块,num_blocks = 200
    # 迭代处理每个块
    ...
    for start_n in range(0, num_blocks, 1):
        ...
        p = tl.exp(qk - m_ij)  # [BLOCK_N] 一个分块的大小，共有5个块
...
def _flash_decoding_stage2_kernel(
    ...
    # stage1 区内的最大值
    # stage2 全局的最大值
    # 在1000 个区，每个区有5个小块，分成200个块,num_partitions = 200
...
if __name__ == "__main__":
    ...
    plot_benchmark_results(results, fix_bs=4, fix_dim=2048)